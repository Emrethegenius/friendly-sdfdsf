...
// ============================================
const QUESTIONS_VERSION = 'v5'; // Bump this when you change the quiz data
const QUIZ_START_DATE = new Date("2025-02-10");

// Immediately check localStorage; clear all cached entries if the day or version has changed.
(function() {
    // Use the current date string as today
    ...
    console.debug('Cache check:', { storedDate, storedVersion, today: todayStr, storedLast });

    // If the cached date is not today's or the version does not match,
    // clear the cache so that a fresh game is started.
    if ( (storedDate && storedDate !== todayStr) || (storedVersion && storedVersion !== QUESTIONS_VERSION) ) {
        ...
        console.debug('Cleared all cached keys.');
    }
})();
...
setInterval(() => {
    ...
    console.log('Interval: Current Date:', todayStr, 'Stored Date:', storedDate); // ADDED
    console.log('Interval: Local Storage - dailyQuestionsDate:', localStorage.getItem('dailyQuestionsDate'), 'dailyQuestionsVersion:', localStorage.getItem('dailyQuestionsVersion'), 'dailyQuestions:', localStorage.getItem('dailyQuestions')); // ADDED

    if (!storedDate || storedDate !== todayStr) {
        console.log('Interval: Date mismatch - Clearing cache and reloading'); // ADDED
        localStorage.removeItem('dailyQuestions');
        localStorage.removeItem('dailyQuestionsDate');
        localStorage.removeItem('dailyQuestionsVersion');
        localStorage.removeItem('gameState');
...
function checkMidnightReset() {
    ...
    console.log('Midnight Reset: Time until midnight:', timeUntilMidnight); // ADDED

    setTimeout(() => {
        console.log('Midnight Reset: Resetting cache and reloading'); // ADDED
        localStorage.removeItem('dailyQuestions');
        localStorage.removeItem('dailyQuestionsDate');
        localStorage.removeItem('dailyQuestionsVersion');
        localStorage.removeItem('gameState');
...
function endGame() {
    ...
    console.log('End Game: Game state saved:', gameState); // ADDED
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        startButton.onclick = function () {
            console.log('Start Button: Clicked'); // ADDED
            const savedState = JSON.parse(localStorage.getItem('gameState') || '{}');
            console.log('Start Button: Saved State:', savedState); // ADDED
            console.log('Start Button: canPlayToday():', canPlayToday()); // ADDED
            console.log('Start Button: Local Storage - dailyQuestionsDate:', localStorage.getItem('dailyQuestionsDate'), 'dailyQuestionsVersion:', localStorage.getItem('dailyQuestionsVersion'), 'dailyQuestions:', localStorage.getItem('dailyQuestions')); // ADDED
